...
VKTL_EXPORT_ namespace vktl::math {
    ...
    using extents = ::std::index_sequence<ids...>;

    template<typename T = float>
    struct vec1 {
        static constexpr::std::size_t row = 1u;
        static constexpr::std::size_t col = 0u;
    ...
    template<typename T = float>
    struct vec2 {
        static constexpr::std::size_t row = 2u;
        static constexpr::std::size_t col = 0u;
        using extent = extents<row>;
        using type = T[row];
        union {
    ...
    template<typename T = float>
    struct vec3 {
        static constexpr::std::size_t row = 3u;
        static constexpr::std::size_t col = 0u;
        using extent = extents<row>;
        using type = T[row];
        union {
    ...
    template<typename T = float>
    struct vec4 {
        static constexpr::std::size_t row = 4u;
        static constexpr::std::size_t col = 0u;
        using extent = extents<row>;
        using type = T[row];
        union {
    ...
    using vec1u32 = vec1u;
    using u32x1 = vec1u;
    using vec1u16 = vec1<::std::uint16_t>;
    using u16x1 = vec1u16;
    using vec1u8 = vec1<::std::uint8_t>;
    using u8x1 = vec1u8;
    ...
    struct mat1x1 {
        static constexpr ::std::size_t row = 1u;
        static constexpr ::std::size_t col = 1u;

        using extent = extents<row, col>;

        using vec = T[col]; using vec_T = T[row];
        using type = T[row][col]; using transpose = T[col][row];
        ...
        constexpr auto& operator()(::std::size_t x, ::std::size_t y) const noexcept { return value[x][y]; }
        constexpr auto& operator()(::std::size_t x, ::std::size_t y) noexcept { return value[x][y]; }
    };

    template<typename T = float>
    struct mat1x2 {
        static constexpr ::std::size_t row = 1u;
        static constexpr ::std::size_t col = 2u;
    ...
    struct mat1x3 {
        static constexpr ::std::size_t row = 1u;
        static constexpr ::std::size_t col = 3u;
        ...
        union {
            struct { T m11, m12, m13; };
            struct { vec1<T> v1, v2, v3; };
            vec1<T> v[col];
            type value;
        };
    ...
    struct mat1x4 {
        static constexpr ::std::size_t row = 1u;
        static constexpr ::std::size_t col = 4u;
    ...
    struct mat2x3 {
        ...
        union {
            struct { T m11, m21, m12, m22, m13, m23; };
            struct { vec2<T> v1, v2, v3; };
            vec2<T> v[col];
            type value;
        };
    ...
    struct mat2x4 {
        ...
        union {
            struct { T m11, m21, m12, m22, m13, m23, m14, m24; };
            struct { vec2<T> v1, v2, v3, v4; };
            vec2<T> v[col];
            type value;
        };
    ...
    struct mat3x2 {
        ...
        union {
            struct { T m11, m21, m31, m12, m22, m32; };
            struct { vec3<T> v1, v2; };
            vec3<T> v[col];
            type value;
        };
    ...
    struct mat3x4 {
        ...
        union {
            struct { T m11, m21, m31, m12, m22, m32, m13, m23, m33, m14, m24, m34; };
            struct { vec3<T> v1, v2, v3, v4; };
            vec3<T> v[col];
            type value;
        };
    ...
    struct mat4x2 {
        ...
        union {
            struct { T m11, m21, m31, m41, m12, m22, m32, m42; };
            struct { vec4<T> v1, v2; };
            vec4<T> v[col];
            type value;
        };
    ...
    struct mat4x3 {
        ...
        union {
            struct { T m11, m21, m31, m41, m12, m22, m32, m42, m13, m23, m33, m43; };
            struct { vec4<T> v1, v2, v3; };
            vec4<T> v[col];
            type value;
        };
    ...
    struct view {
        ...
        : value{ mat.value }
            , stride_x{ ::std::remove_cvref_t<M>::row }
            , stride_y{ ::std::remove_cvref_t<M>::col }
        {
        }

        template<typename M>
        constexpr view(M& mat, ::std::uint32_t offset_x) noexcept
            requires(requires{ typename::std::remove_cvref_t<M>::extents; })
        : view(mat)
            , offset_x{ offset_x }
        {
        ...
        template<typename M>
        constexpr view(M& mat, ::std::uint32_t offset_x, ::std::uint32_t offset_y) noexcept
            requires(requires{ typename::std::remove_cvref_t<M>::extents; })
        : view(mat)
            , offset_x{ offset_x }
            , offset_y{ offset_y }
    ...
    private:
        ...
        ::std::uint32_t stride_x = 0;
        ::std::uint32_t stride_y = 0;
    };

    template<typename T>
    using vec_view = view<T>;
    template<typename T>
    using mat_view = view<T>;
}