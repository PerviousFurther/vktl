...
VKTL_EXPORT_ namespace vktl::detail {
  ...
  template <typename N> struct m<swapchain, N> : N {
    constexpr m(swapchain const &sw, auto &&...others)
        : N{forward_(others)...},
          info{
              ...
              .imageExtent = {.width = sw.width, .height = sw.height},
              .imageArrayLayers = 1,
              .imageUsage = VK_
                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, // some may allow depth
                                                       // stencil, but this is
                                                       // normal usage.
    ...
    template <typename Fn = ::std::nullptr_t> void init(Fn &&fn = nullptr) {
      N::init();
      ...
      if (!handle_) {
        ...
        if constexpr (::std::invocable<Fn &, VK_ VkSwapchainCreateInfoKHR &,
                                       VK_ VkSurfaceCapabilities2EXT &>) {
          fn(info, capabilities);
          VKTL_ASSERT(info.minImageCount); // atleast image.
          VKTL_ASSERT(info.imageExtent.width &&
                      info.imageExtent.height); // atleast width and height.
        }

        if constexpr (::std::invocable<Fn &, VK_ VkSwapchainCreateInfoKHR &,
                                       vector<VK_ VkSurfaceFormatKHR> &>) {
          fn(info, formats);
          VKTL_ASSERT(info.imageFormat != VK_ VK_FORMAT_MAX_ENUM);
          VKTL_ASSERT(info.imageColorSpace != VK_ VK_COLOR_SPACE_MAX_ENUM_KHR);
        } else if (info.imageFormat == VK_ VK_FORMAT_MAX_ENUM ||
                   info.imageColorSpace == VK_ VK_COLOR_SPACE_MAX_ENUM_KHR) {
          vkget(formats, VK_ vkGetPhysicalDeviceSurfaceFormatsKHR, phydv,
                win->handle()) |
        ...
        if constexpr (::std::invocable<Fn &, VK_ VkSwapchainCreateInfoKHR &,
                                       vector<VK_ VkPresentModeKHR> &>) {
          fn(info, present_modes);
          VKTL_ASSERT(info.imageColorSpace != VK_ VK_COLOR_SPACE_MAX_ENUM_KHR);
        } else if (info.presentMode == VK_ VK_PRESENT_MODE_MAX_ENUM_KHR) {
          vkget(present_modes, VK_ vkGetPhysicalDeviceSurfacePresentModesKHR,
                phydv, win->handle()) |
        ...
        VK_ vkCreateSwapchainKHR(dv->handle(), &info, N::allocator(),
                                 &handle_.value) |
    ...
    constexpr auto frame_index() const noexcept { return ensure_frame_index(); }
    ...
    uint64_t frame_revision(uint32_t frame) const noexcept {
      VKTL_ASSERT(frame < frame_revisions_.size());
      return frame_revisions_[frame];
    }

    void fence(object_of<fence> auto &object) { fence_ = {object}; }
  ...
  private:
    ...
    uint16_t ensure_frame_index() const {
      if (!fence_.empty()) {
        if (auto handle = fence_.handle())
          VKTL_LIKELY {
            VK_ vkWaitForFences(handle_of<device>(this), 1u, &handle, false,
                                maximum);
          }
      }
