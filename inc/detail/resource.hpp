...
VKTL_EXPORT_ namespace vktl::vptr {
  template <typename H = memory_handle> struct freeable {
    using handle_info = H;
    ...
    template <typename C> struct apply : C {
      using base = C;
      ...
      void free(handle_info handle) {
        vptr.free_suballoc_(C::get_this(), handle);
      }
  ...
  template <typename Trait> struct memory_resource {
    ...
    template <typename T> constexpr void rebind() noexcept {
      memories_flags_ = nullptr;
      if constexpr (requires(T &v) {
                      {
                        v.memory_flags()
                      } -> ::std::convertible_to<
                          span<default_memory_flags const>>;
                    }) {
        memories_flags_ =
            [](void const *ptr) -> span<default_memory_flags const> {
          return static_cast<T const *>(ptr)->memory_flags();
        };
      } else if constexpr (requires(T &v) {
                             {
                               v.memory_flags()
                             } -> ::std::convertible_to<default_memory_flags>;
                           }) {
        memory_flags_ = [](void const *ptr) -> default_memory_flags {
          return static_cast<T const *>(ptr)->memory_flags();
        };
      } else {
        memory_flags_ = [](void const *) { return default_memory_flags{}; };
      }
#if defined(VK_KHR_get_memory_requirements2)
      memory_requirement_ = nullptr;
      if constexpr (requires(T const &v, ext_memreq &value) {
                      v.memory_requirement(value, 0u);
                    }) {
        memory_requirement_ = [](void const *ptr, ext_memreq &value,
                                 uint32_t index) {
          static_cast<T const *>(ptr)->memory_requirement(value, index);
        };
      } else if constexpr (requires(T const &v, ext_memreq &value) {
                             v.memory_requirement(value);
                           }) {
        memory_requirement_ = [](void const *ptr, ext_memreq &value, uint32_t) {
          static_cast<T const *>(ptr)->memory_requirement(value);
        };
      }
#endif
      if constexpr (requires(T const &v) {
                      { v.is_tiling() } -> ::std::convertible_to<bool>;
                    }) {
        is_tiling_ = [](void const *ptr) -> bool {
          return static_cast<T const *>(ptr)->is_tiling();
        };
      } else {
        is_tiling_ = [](void const *) { return false; };
      }
    }
    ...
    template <typename C>
    using base =
        apply_compose<C, bindable<detail::bind_memory>,
                      handle_owner<detail::locked<handle_type>>, frame_indexed>;

    template <typename C> struct apply : base<C> {
      using base = base<C>;
      ...
      default_memory_flags memory_flags() const noexcept {
        return vptr.memory_flags_(C::get_this());
      }
      span<default_memory_flags const> memories_flags() const noexcept {
        return vptr.memories_flags_(C::get_this());
      }
...
VKTL_EXPORT_ namespace vktl::detail {
...
#if defined(VK_KHR_get_memory_requirements2)
  inline void requirement(VK_ VkDevice device, VK_ VkImage handle,
                          VK_ VkMemoryRequirements2KHR &reqs) {
  ...
  inline void requirement(VK_ VkDevice device, VK_ VkBuffer handle,
                          VK_ VkMemoryRequirements2KHR &reqs) {
  ...
  inline VK_ VkMemoryRequirements const &
  requirement(VK_ VkMemoryRequirements2KHR const &reqs) {
    return reqs.memoryRequirements;
  }
#endif
  inline VK_ VkMemoryRequirements const &
  requirement(VK_ VkMemoryRequirements const &reqs) {
    return reqs;
  }
  } // namespace memory
  ...
  struct default_memory : memory_handle_tag {
    ...
    default_memory(auto pthis) : parent{pthis} {}
  ...
  template <typename N> struct m<memory_allocator_, N> : N {
    using base = N;
  ...
  protected:
    void reset() {
      ...
      for (default_memory &memory : this->memories) {
        VKTL_ASSERT(
            ...
            memory::dedicated_allocation); // test, normally, resource should
                                           // already release when reset.
    ...
    auto get_state(void *pprops = nullptr, void *plimits = nullptr) {
      ...
      if (pprops) {
        auto *p2 =
            static_cast<VK_ VkPhysicalDeviceMemoryProperties2KHR *>(pprops);
        p2->sType =
            VK_ VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MEMORY_PROPERTIES_2_KHR;
    ...
    void each(auto const &state, auto &child,
              memory::resource_allocation &alloc, void *pnext = nullptr) {
      auto hdv = handle_of<device>(this);
    ...
    void find(auto const &state, memory::resource_allocation &prealloc) {
      ...
      if (!type_bits) {
        throw error{VK_ VK_ERROR_FEATURE_NOT_PRESENT,
                    "[MEMORY ALLOCATOR] Cannot allocate memory satisfied with "
                    "specified resource."};
      }
      ...
      for (; i < properties.memoryTypeCount; ++i) {
        if ((type_bits & i) != 0u &&
            (properties.memoryTypes[i].propertyFlags & prop_flags) ==
                prop_flags &&
      ...
      if (i >= properties.memoryTypeCount) {
        throw error{VK_ VK_ERROR_FEATURE_NOT_PRESENT,
                    "[MEMORY ALLOCATOR] Cannot allocate memory suitable for "
                    "specified resource."};
      }
    }
  ...
  //
  // one version is for directly allocate -> for dedicated or other memory
  // decision. other version is for resource bundle -> express instance of
  // `memory::allocate`.
  ...
  template <typename N> struct m<budget_, N> : N {
    constexpr m(budget_, auto &&...others) : N{forward_(others)...} {
      parent_of<instance>(this)->append_extensions(
          VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME, 1u);
  ...
  protected:
    auto get_state(void *pprops = nullptr, void *plimits = nullptr) {
      ...
      return ::std::tuple_cat(::std::tuple(::std::move(budget)),
                              ::std::move(state));
    }

    void allocate(auto &state, auto &child, memory::allocate &info,
                  void *pnext = nullptr) {
      N::allocate(state, child, info, pnext);
      check_budget(state, info.memoryTypeIndex);
    }
    void allocate(auto &state, auto &child, memory::resource_allocation &info,
                  void *pnext = nullptr) {
      N::allocate(state, child, info, pnext);
  ...
  private:
    void check_budget(auto &state, uint32_t type_index, size_t size) {
      ...
      if (usage <= size_budget) {
        ...
        throw error{
            VK_ VK_ERROR_OUT_OF_DEVICE_MEMORY,
  ...
  template <typename N> struct m<budget_, N> : N {
    static_assert(always_false<N>,
                  "Vulkan header not supporting "
                  "VK_KHR_get_memory_requirements2 cannot use `budget`.");
  };
#endif
  ...
  struct dedicated_memory : default_memory {
    dedicated_memory(auto pthis) : default_memory{pthis} {
      allocation_type = memory::dedicated_allocation;
    }
  };

  template <typename N> struct m<dedicated_, N> : N {
  protected:
    constexpr m(dedicated_, auto &&...others) : N{forward_(others)...} {
      auto dv = parent_of<device>(this);
      dv->append_extensions(VK_KHR_DEDICATED_ALLOCATION_EXTENSION_NAME, 1);
    }

    void each(auto &state, auto &child, memory::resource_allocation &info,
      ...
      if (/*req.prefersDedicatedAllocation || */ req
    ...
    void allocate(auto &state, auto &child,
                  memory::resource_allocation const &info,
                  void *pnext = nullptr) {
      if ((info.alloc_mask & memory::dedicated_bit) != 0u) {
        ...
        try {
          VK_ vkAllocateMemory(handle_of<device>(this), &alloc_info,
                               N::allocator(), &memory.handle) |
  ...
  public:
    ...
    void free(bind_memory bind) {
      if (bind.memory) {
        VK_ vkFreeMemory(handle_of<device>(this), bind.memory->handle,
                         N::allocator());
  ...
  template <typename N> struct m<dedicated_, N> : N {
    static_assert(always_false<N>,
                  "Vulkan header not supporting VK_KHR_dedicated_allocation "
                  "cannot use `dedicated`.");
  };
#endif
  ...
  template <typename P> struct paged_memory : default_memory {
    using page_type = P;
    ...
    size_t allocate(size_t size, size_t alignment, size_t granularity,
                    bool is_tiling) {
    ...
    bool has_space(size_t size, size_t alignment, bool is_tiling) const {
      for (const auto &p : pages) {
        if (p.is_tiling == is_tiling &&
            p.has_space(size, alignment, min_block_size)) {
          return true;
        }
      }
      return false;
  ...
  private:
    ...
    size_t allocate_impl(size_t size, size_t alignment, size_t granularity,
                         bool is_tiling) {
      if constexpr (allow_grow) {
        if (pages.empty() || pages.back().is_tiling != is_tiling) {
          ...
          if (!pages.empty()) {
            const size_t prev_end =
                pages.back().offset + pages.back().total_size;
          ...
          size_t local_offset = target_page.template allocate<true>(
              size, alignment, min_block_size);
        ...
        for (auto &p : pages) {
          if (p.is_tiling == is_tiling) {
            size_t local_offset =
                p.template allocate<false>(size, alignment, min_block_size);
            if (local_offset != invalid) {
  ...
  template <typename N, typename M> struct basic_memory_allocator : N {
  protected:
    basic_memory_allocator(auto &&...others) : N{forward_(others)...} {}

  public:
    void free(bind_memory memory) {
      VKTL_ASSERT(
          memory.handle() &&
          static_cast<default_memory *>(memory.memory)->allocation_type ==
              M::page_type::allocation_type);
  ...
  protected:
    void init(auto &&...resources) {
      ...
      for (memory::alloc_group &group : groups) {
        VKTL_ASSERT(group.allocation.size()); // for test.

        M &memory = N::memories.template emplace_back<M>(this);
        for (memory::resource_allocation &alloc : group.allocation) {
          alloc.offset =
              memory.allocate(alloc.size, alloc.alignment,
                              limits.bufferImageGranularity, alloc.is_tiling);
        }
    ...
    auto each(auto &state, uint8_t resource_index, auto &childs) {
        ...
        for (auto flag : sflags) {
          ...
          if (result.directly_allocate) {
            ...
            auto it = ::std::ranges::find_if(
                infos, [type_index](memory::alloc_group const &group) {
      ...
      ::std::ranges::stable_partition(
          infos, [](auto const &value) { return !value.is_tiling; });
      return infos;
    }
    ...
    auto each(auto &state, uint32_t index, auto &values, auto &...resources) {
      vector<memory::alloc_group> others =
          each(state, index + 1u, resources...);
      vector<memory::alloc_group> groups = each(state, index, values);
      for (memory::alloc_group const &group : groups) {
        auto it = ::std::ranges::lower_bound(
            others, [type_index = group.type_index](auto const &info) {
        ...
        auto group_tiling_it = ::std::ranges::find_if(
            group.allocation, [](auto const &val) { return val.is_tiling; });
        auto allocs_tiling_it = ::std::ranges::find_if(
            allocs, [](auto const &val) { return val.is_tiling; });
        allocs.insert(allocs_tiling_it, group.allocation.begin(),
                      group_tiling_it);
        allocs.insert(allocs.end(), group_tiling_it, group.allocation.end());
      }
      return others;
    }
    auto each(auto &...resources) { each(0u, resources...); }

    auto bind(auto &memory, memory::alloc_group const &group,
              auto &...resources) {
      for (auto &allocation : group.allocation) {
        invoke_by_index(
            allocation.childs_index,
            [&](auto &childs) {
              childs[allocation.resource_index].bind(
                  bind_memory{.memory = &memory,
  ...
  //
  // TODO: these memory implmentation is not checked, they might have lots of
  // bugs.
  //
  ...
  struct page_buddy : page_base {
    ...
    uint32_t size_to_order(size_t size, size_t alignment,
      ...
      return static_cast<uint32_t>(::std::bit_width(rounded) -
                                   ::std::bit_width(min_block_size));
    }
    ...
    bool has_space(size_t size, size_t alignment, size_t min_block_size) const {
      return has_available_block(
          size_to_order(size, alignment, min_block_size));
    }
    ...
    size_t allocate(size_t size, size_t alignment, size_t min_block_size) {
      ...
      while (k > order) {
        --k;
        const size_t buddy_offset =
            offset_in_page + block_size(k, min_block_size);
  ...
  struct page_best_fit : page_base {
    ...
    size_t allocate_no_grow(size_t size, size_t alignment = 1,
                            size_t min_block_size = 256) {
    ...
    void free(size_t offset, size_t size, size_t min_block_size = 256) {
      ...
      if (next_it != free_by_offset.end() &&
          (offset + size == next_it->first)) {
  ...
  public:
    bool has_space(size_t size, size_t alignment = 1,
                   size_t min_block_size = 256) const {
      return find_best_fit(size, alignment) != free_by_size.end();
    }

    void free_from_front(size_t offset, size_t size,
                         size_t min_block_size = 256) {
      free(offset, size, min_block_size);
    }
    void free_from_behind(size_t offset, size_t size,
                          size_t min_block_size = 256) {
      free(offset, size, min_block_size);
    }
  ...
  struct page_linear : page_base {
    ...
    bool has_space(size_t size, size_t alignment = 1,
      ...
      return (aligned_cursor <= total_size) &&
             (size <= total_size - aligned_cursor);
    }

    void free(size_t offset, size_t size, size_t min_block_size = 256) {
      auto it = ::std::lower_bound(
          allocations.begin(), allocations.end(), offset,
          [](const allocation &a, size_t offset) { return a.offset < offset; });

      if (it != allocations.end() && it->offset == offset && it->size == size) {
    ...
    void free_from_front(size_t offset, size_t size,
                         size_t min_block_size = 256) {
      if (!allocations.empty() && allocations.front().offset == offset &&
    ...
    void free_from_behind(size_t offset, size_t size,
                          size_t min_block_size = 256) {
      if (!allocations.empty() && allocations.back().offset == offset &&
  ...
  struct m<buddy_, N> : basic_memory_allocator<N, buddy_memory> {
    ...
    constexpr m(buddy_, auto &&...others) : base{forward_(others)...} {}
  };
  template <typename N>
  struct m<best_fit_, N> : basic_memory_allocator<N, best_fit_memory> {
    using base = basic_memory_allocator<N, best_fit_memory>;
    constexpr m(best_fit_, auto &&...others) : base{forward_(others)...} {}
  };
  template <typename N>
  struct m<arena_, N> : basic_memory_allocator<N, linear_memory> {
    using base = basic_memory_allocator<N, linear_memory>;
    constexpr m(arena_, auto &&...others) : base{forward_(others)...} {}
  };

  template <typename N, typename Trait> struct basic_allow_resource : N {
    constexpr basic_allow_resource(auto &&...infos) : N{forward_(infos)...} {}
  ...
  template <typename N>
  struct m<memory_allocator_extensions::allow_buffer_, N>
      : basic_allow_resource<N, trait<buffer>> {
    using base = basic_allow_resource<N, trait<buffer>>;
    m(allow_buffer_, auto &&...others) : base{forward_(others)...} {}
  };

  template <typename N>
  struct m<memory_allocator_extensions::allow_image_, N>
      : basic_allow_resource<N, trait<image>> {
    using base = basic_allow_resource<N, trait<image>>;
    m(allow_image_, auto &&...others) : base{forward_(others)...} {}
  };
  ...
  struct basic_memory_resource : basic_frame_indexed_handle<N, Trait> {
    using base = basic_frame_indexed_handle<N, Trait>;
    using handle_type = typename Trait::handle_type;

    constexpr basic_memory_resource(auto &&...others)
        : base{forward_(others)...} {}
    ...
    void bind(bind_memory const &memory) noexcept {
      auto _ = locker_of(this);
      VKTL_ASSERT(!memories_.empty());
      VKTL_ASSERT(
          ::std::ranges::all_of(memories_.column<1u>(), [](auto const &value) {
            return value.empty();
          })); // internal test, resource cannot bind twice.
      VKTL_ASSERT(memory.index <
                  this->frame_count()); // internal test, out of range usually.
      Trait::bind_memory(handle_of<device>(this), this->handle(memory.index),
                         memory.handle(), memory.offset) |
          popup{"[BIND MEMORY] Bind memory failure."};
    }

  protected:
    void reset() {
      for (auto &[memory, binds] : memories_) {
        for (bind_memory &bind : binds) {
          bind.free();
        }
        binds.clear();
      }
    ...
    // ptrs should [subres_index * frame_count], and can be access by
    // [frame_index * subres_count + subres_index].
    void bind(bind_memory const &memory, span<void *> ptrs) noexcept {
      ...
      memories_.column<1u>()[memory.index / memories_.size()]
                            [memory.index % frame_count] = memory;
      if (memory.index == memories_.size() * frame_count) {
        ...
        for (auto frame_index = 0u; frame_index < frame_count; frame_index++) {
          ...
          for (auto pnext :
               ptrs.subspan(index, index + this->memories_.size())) {
            bind_memory &memory =
                memories_.column<1u>()[subres_index][frame_index];
            binds.emplace_back(
                Trait::memory_info(pnext, this->handle(frame_index),
                                   memory.handle(), memory.offset));
            subres_index++;
          }
          Trait::bind_memory_2(handle_of<device>(this), uint32_t(binds.size()),
                               binds.data()) |
              popup{"[BIND MEMORY] Bind memory failure."};
          binds.clear();
        }
    ...
    void memory_flags(default_memory_flags flags,
                      uint32_t subres_index = 0u) noexcept {
  ...
  template <typename N> struct m<mappable, N> : N {
    constexpr m(mappable props, auto &&...others) : N{forward_(others)...} {
      // props.cache ? VK_ VK_MEMORY_PROPERTY_HOST_CACHED_BIT;
    ...
    void upload(byte_view data, size_t offset = maximum) {
      ...
      if (size > bytes_.size()) {
        VKTL_ASSERT(N::info.size >= size);
        bytes_.resize(size);
      }
      ::std::ranges::copy(data, bytes_.begin() + offset);
    }
