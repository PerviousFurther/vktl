...
VKTL_EXPORT_ namespace vktl::detail {
  template <typename N> struct basic_window : N {
    constexpr basic_window(auto &&...infos) : N{forward_(infos)...} {}
...
VKTL_EXPORT_ namespace vktl::detail {
  template <typename N> struct m<window, N> : basic_window<N> {
    ...
    void init() {
      N::init();
      if (!this->handle_) {
        VKTL_ASSERT(info.hwnd && info.hinstance);
        VK_ vkCreateWin32SurfaceKHR(handle_of<instance>(this), &info,
                                    N::allocator(), &this->handle_.value) |
    ...
    void reset() {
      if (this->handle_) {
        VK_ vkDestroySurfaceKHR(handle_of<instance>(this),
                                ::std::exchange(this->handle_, nullptr),
                                N::allocator());
      }
    }

    uint32_t width() const noexcept {
      VKTL_ASSERT(VK_ IsWindow(info.hwnd)); // test.
      VK_ RECT rt; VK_ GetClientRect(info.hwnd, &rt);
      return uint32_t(rt.right - rt.left);
    }

    uint32_t height() const noexcept {
      VKTL_ASSERT(VK_ IsWindow(info.hwnd));
      VK_ RECT rt; VK_ GetClientRect(info.hwnd, &rt);
...
VKTL_EXPORT_ namespace vktl::detail {
  template <typename N> struct m<window, N> : basic_window<N> {
    ...
    void allocate() {
      N::allocate();
      if (!this->handle_) {
        VKTL_ASSERT(info.window);
        VK_ vkCreateAndroidSurfaceKHR(handle_of<instance>(this), &info,
                                      N::allocator(), &this->handle_.value) |
            popup("[WINDOW] Create surface failure.");
      }
    }
    ...
    uint32_t width() const noexcept {
      VKTL_ASSERT(info.window); // internal error.
      return uint32_t(VK_ ANativeWindow_getWidth(info.window));
    }

    uint32_t height() const noexcept {
      VKTL_ASSERT(info.window); // internal error.
      return uint32_t(VK_ ANativeWindow_getHeight(info.window));
    }
...
VKTL_EXPORT_ namespace vktl::detail {
  template <typename N> struct m<window, N> : basic_window<N> {
    ...
    constexpr m(window win, auto &&...others)
        : base{forward_(others)...}, info{make_create_info(win)} {
      parent_of<instance>(this)->append_extensions(
          VK_KHR_SURFACE_EXTENSION_NAME);
#if defined(VKTL_USE_WAYLAND)
      parent_of<instance>(this)->append_extensions(
  ...
  private:
    static constexpr create_info_type make_create_info(window win) noexcept {
      ...
      return {
          ...
          .window = static_cast<VK_ xcb_window_t>(
              reinterpret_cast<uintptr_t>(win.handle)),
      };
#endif
    }
    ...
    void create_surface() {
#if defined(VKTL_USE_WAYLAND)
      VKTL_ASSERT(info.display && info.surface);
      VK_ vkCreateWaylandSurfaceKHR(handle_of<instance>(this), &info,
                                    N::allocator(), &this->handle_.value) |
          popup("[WINDOW] Create surface failure.");
#else
      VKTL_ASSERT(info.connection && info.window);
      VK_ vkCreateXcbSurfaceKHR(handle_of<instance>(this), &info,
                                N::allocator(), &this->handle_.value) |
          popup("[WINDOW] Create surface failure.");
#endif
    }
    ...
    size_2d query_geometry() const noexcept {
      if (!info.connection || !info.window)
        return {};
...
VKTL_EXPORT_ namespace vktl::detail {
  template <typename N> struct m<window, N> : basic_window<N> {
    ...
    void allocate() {
      N::allocate();
      if (!this->handle_) {
        VKTL_ASSERT(info.pLayer);
        VK_ vkCreateMetalSurfaceEXT(handle_of<instance>(this), &info,
                                    N::allocator(), &this->handle_.value) |
            popup("[WINDOW] Create surface failure.");
      }
    }
