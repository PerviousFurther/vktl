...
VKTL_EXPORT_ namespace vktl::detail {
  ...
  struct bind_descriptors {
    ...
    void *info; // since it might have descriptor buffer or descriptor heap,
                // leave it void*.
    ...
    span<VK_ VkDescriptorSet> sets() const noexcept;
    void free();

    constexpr operator bool() noexcept { return handle; }
  };
}
  ...
  struct descriptor_sets {
    ...
    constexpr void rebind() noexcept
      requires(requires(T const &v) {
        { v.layout_infos() } -> ::std::convertible_to<layout_infos>;
        { v.layouts() } -> ::std::convertible_to<layout_handles>;
      })
    {
      infos_ = [](void const *ptr) -> layout_infos {
        return static_cast<T const *>(ptr)->layout_infos();
      };
      layouts_ = [](void const *ptr) noexcept -> layout_handles {
        return static_cast<T const *>(ptr)->layouts();
      };
    }

    vfn<layout_handles()> layouts_ = nullptr;
    vfn<layout_infos() const noexcept> infos_ = nullptr;
  };

  template <typename C> struct descriptor_sets::apply : base<C> {
    using base = base<C>;
    ...
    layout_infos infos() const noexcept { return vptr.infos_(C::get_this()); }
...
VKTL_EXPORT_ namespace vktl::detail {
  ...
  template <typename N> struct m<descriptor_allocator_, N> : N {
  ...
  using pool_sizes =
      vectors<VK_ VkDescriptorPoolSize, vector<VK_ VkDescriptorType>>;
  ...
  insert_pool_size(pool_sizes &vec, VK_ VkDescriptorPoolSize const &binding) {
    bool result = false;
    auto it = ::std::ranges::find_if(
        vec, [&](auto const &p) { return get<0u>(p).type == binding.type; });
    if (it == vec.end()) {
      it = vec.insert(it,
                      VK_ VkDescriptorPoolSize{
                          .type = binding.type,
                      },
                      by_default);
      result = true;
  ...
  inline constexpr bool
  insert_pool_size(pool_sizes &vec,
  ...
  inline constexpr bool
  insert_pool_size(pool_sizes &vec, VK_ VkDescriptorPoolSize const &binding,
                   vector<VK_ VkDescriptorType> const &types) {
    bool result = false;
    auto it = ::std::ranges::find_if(
        vec, [&](auto const &p) { return get<1u>(p) == types; });
    if (it == vec.end()) {
      it = vec.insert(it,
                      VK_ VkDescriptorPoolSize{
                          .type = binding.type,
  ...
  struct descriptor_pool : default_descriptor {
    descriptor_pool(auto &parent) : default_descriptor{parent} {
      type = descriptor::type_pool;
    }

    ~descriptor_pool() {
      VKTL_ASSERT(!pool); // descriptor leakage.
    }
    ...
    descriptor::pool_sizes
        pool_sizes;  // rest pool size, enable when allow free.
    poly_list infos; // alloc infomation. use `poly_list` since `list` cannot
                     // erase from element in O(1).
  };

  // N ususally m<descriptor_allocator_, <other>>
  template <typename N> struct m<from_set_, N> : N {
    m(from_set_, auto &&...others) : N{forward_(others)...} {}

    void allocate() { allocate(VK_ VkDescriptorPoolCreateFlags(0u)); }
    ...
    void free(bind_descriptors const &bind) {
      auto _ = locker_of(this);
      if (bind.handle) {
        VKTL_ASSERT(bind.info); // test.
        auto &pool = *static_cast<descriptor_pool *>(bind.handle);
        auto &info =
            *static_cast<descriptor::alloc_set_info const *>(bind.info);
        if ((pool.flags &
             VK_ VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT) != 0u) {
          ...
          VK_ vkFreeDescriptorSets(handle_of<device>(this), pool.pool,
                                   info.count, sets.data()) |
  ...
  private:
    void free(descriptor_pool &pool) {
      assert(pool.pool); // test.
      VK_ vkDestroyDescriptorPool(handle_of<device>(this), pool.pool,
                                  N::allocator());
      pool.sets.clear();
      pools_.erase(pool);
    }
    ...
    void allocate(VK_ VkDescriptorPoolCreateFlags pool_flags = 0u) {
      N::init();

      const bool allow_free =
          (pool_flags & VK_ VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT);
      ...
      descriptor::pool_sizes
          sizes;               // [vk pool size, descriptor types]. per types.
      ...
      for (auto &child : childs_) {
        ...
        auto &alloc =
            alloc_results.template emplace_back<descriptor::alloc_set_info>();
        alloc.offset = index;
        for (default_descriptor_set_layout const &layout_info : child.infos()) {
          ...
          for (auto const &info : layout_info.layouts) {
            ...
            if ((flags & VK_ VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT_EXT) !=
                0u) {
              pool_flags |=
                  VK_ VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT_EXT;
            }
            if ((flags &
                 VK_ VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT_EXT) !=
                0u) {
              VKTL_ASSERT(
                  variable_count ==
                  0u); // test, if trigger this problem, release an issue.
            ...
            if (binding.descriptorType ==
                VK_ VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK_EXT) {
...
#endif
            // since fill alloc will only invoke once, multiple with frame
            // count.
...
#else
              (void)descriptor::insert_pool_size(sizes, bindings);
#endif
              if (allow_free) {
                (void)descriptor::insert_pool_size(alloc.pool_sizes, binding);
              }
      ...
      VK_ VkMutableDescriptorTypeCreateInfoVALVE mutable_info{
          .sType =
              VK_ VK_STRUCTURE_TYPE_MUTABLE_DESCRIPTOR_TYPE_CREATE_INFO_VALVE,
          .pNext = pool_info.pNext,
      ...
      if (have_mutable_descriptor) {
        for (auto &&[types, size] :
             spans{type_lists, sizes}) { // only assign, is safe.
      ...
      descriptor_pool &pool =
          pools_.template emplace_back<descriptor_pool>(this);
      auto hdv = handle_of<device>(this);
      ...
      if (have_varaible_descriptor_count) {
        variable_alloc_info.pNext =
            ::std::exchange(alloc_info.pNext, &variable_alloc_info);
      }
#endif
      try {
...
VKTL_EXPORT_ namespace vktl::detail {
  inline span<VK_ VkDescriptorSet> bind_descriptors::sets() const noexcept {
