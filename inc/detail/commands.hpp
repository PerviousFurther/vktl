...
VKTL_EXPORT_ namespace vktl::cmd {
  ...
  using detail::commands;

  template <typename Pass> struct bind_pass {
    Pass &pass;
  };
  ...
  struct bind_vertex_buffer : ::std::tuple<Buffers *...> {
    ...
    bind_vertex_buffer(Args &...args) : base{&args...} {}
  };
  template <typename... Args>
  bind_vertex_buffer(Args & ...) -> bind_vertex_buffer<Args...>;

  template <typename Buffer> struct bind_index_buffer {
    Buffer &object;
  ...
  template <typename SyncObject> struct wait {
    SyncObject &object;
  };
  template <typename SyncObject> struct signal {
    SyncObject &object;
  };
...
VKTL_EXPORT_ namespace vktl::detail {
  ...
  struct begin_command {
    VK_ VkCommandBuffer cmdbuf;
  };

  template <typename N>
  struct m<begin_command, N> : N {
    constexpr m(begin_command, auto &&...others) : N{forward_(others)...} {}
  ...
  struct m<cmd::bind_pass<P>, N> : N {
    constexpr m(cmd::bind_pass<P> const &pass, auto &&...others)
        : N{forward_(others)...}, pass_{&pass.pass} {}

    void begin(
        VK_ VkSubpassContents content = VK_ VK_SUBPASS_CONTENTS_INLINE) const {
      N::begin();
  ...
  protected:
    ...
    VK_ VkRenderingInfoKHR rendering_info_{
  ...
  private:
    ...
    void bind(::std::index_sequence<Indices...>) const {
      // TODO: use locked range.
      VK_ VkBuffer handles[]{::std::get<Indices>(info_)->host().value...};
      VK_ VkDeviceSize offsets[sizeof...(Buffers)]{};
      VK_ vkCmdBindVertexBuffers(N::cmdbuf(), info_.first_binding,
                                 uint32_t(sizeof...(Buffers)), handles,
                                 offsets);
    }
  ...
  struct m<cmd::bind_index_buffer<Buffer>, N> : N {
    ...
    void begin() const {
      N::begin();
      // TODO: it currently not use view.
  ...
  template <typename N> struct m<cmd::bind_pipe, N> : N {
  ...
  struct m<cmd::wait<SyncObject>, N> : N {
    ...
    void begin() const {
      ...
      VK_ vkCmdWaitEvents(N::cmdbuf(), 1u, &event,
                          ...
                          VK_ VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0u, nullptr,
  ...
  // 			((VK_ VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT <<
  // 1u) - 1u) & info.flags);
  // 	}
  //
  // 	void begin() const {
